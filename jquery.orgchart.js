...
(function (factory) {
  ...
  var OrgChart = function (elem, opts) {
    ...
    this.defaultOptions = {
      ...
      'zoominLimit': 7,
      'zoomoutLimit': 0.5,
		/*my extra json attrbiutes*/
		'nodeContent': 'original',
		'nodeUrl' : 'url',
		'nodeNotes' : 'notes',
    };
  };
  ...
  OrgChart.prototype = {
    ...
    loopChart: function ($chart) {
      ...
      var subObj = { 'id': $tr.find('.node')[0].id, 'name': $tr.find('.node')[0].name, 'original': $tr.find('.node')[0].original, 'url': $tr.find('.node')[0].url, 'notes': $tr.find('.node')[0].notes, 'className': $tr.find('.node')[0].className  };
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv.append('<div class="title">' + data[opts.nodeTitle] + '</div>' )
          ...
          .append(typeof opts.nodeUrl !== 'undefined' ? '<div class="url" style="display:none;">' + (data[opts.nodeUrl] || '') + '</div>' : '')
          .append(typeof opts.nodeNotes !== 'undefined' ? '<div class="notes" style="display:none;">' + (data[opts.nodeNotes] || '') + '</div>' : ''); 
      }
      //
